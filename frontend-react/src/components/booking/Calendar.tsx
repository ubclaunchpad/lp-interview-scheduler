...
export default function Calendar(props: Props) {
  ...
  const [highlightedInMonth, setHighlightedInMonth] = React.useState(
    props.highlightDays
  ...
  React.useEffect(() => {
    setHighlightedInMonth(
      props.highlightDays
        .filter((d) => d.year() === year && d.month() === month)
        .map((d) => d.date())
    );
  }, [month, props.highlightDays, year]);
  ...
  const flip = (direction: number) => {
    ...
    setHighlightedInMonth(
      props.highlightDays
        .filter((d) => d.year() === newYear && d.month() === newMonth)
        .map((d) => d.date())
    );
    setSelected(-1);
  };
  ...
  for (let i = 1; i <= numDays; i++) {
    ...
    calendar.push(
      <Day
        date={i}
        onClick={onDayClick}
        highlight={highlightedInMonth.includes(i)}
        selected={selected === i}
        disabled={disabled}
      />
    );
  ...
  return (
    <div className={styles.calendar}>
      ...
      <div className={styles.calendarGrid}>{calendar}</div>
    </div>
  );
}
